# Description
Analyze a file containing statements to map GO IDs to genes and their corresponding statements. This includes grounding mapping, removing duplicates, and comparing annotated genes from GO to the ones from the given statements, optionally plotting the results.
...
def get_genes_for_go_id(goid):
    ...
    params = {'goid': goid, 'format':'tsv', 'db':'UniProtKB', 'tax':'9606', 'col':'proteinSymbol'}
    res = requests.get(quickgo_url, params)
    if not res.status_code == 200:
        logger.error('Could not retrieve proteins associated with GO ID %s' % goid)
        return None
    genes = set([])
    if sys.version_info[0] >= 3:
        ...
        tsv_reader = csv.reader(res.content.splitlines(), delimiter='\t'.encode('utf-8'))
        for row in tsv_reader:
...
def plot_stmt_counts(go_stmt_map, plot_filename, figsize=(3, 3)):
    ...
    for go_id in go_stmt_map.keys():
        counts.append((go_stmt_map[go_id]['names'][0], len(go_stmt_map[go_id]['in_go']), len(go_stmt_map[go_id]['not_in_go'])))
    ...
    plt.legend((p1[0], p2[0]), ('In GO', 'Not in GO'), frameon=False, fontsize=7)
