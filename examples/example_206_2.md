# Description
Process Ligand-Receptor Interactions from OmniPath JSON. Generates Complex and Regulation INDRA Statements.
...
class OmniPathProcessor(object):
    ...
    def _bc_agent_from_up_list(self, up_id_list):
        ...
        agent = agents_list[0]
        agent.bound_conditions = [BoundCondition(a, True) for a in agents_list[1:]]
        return agent
    ...
    def _get_op_complex(self, source, target, evidence_list):
        ag_list = self._complex_agents_from_op_complex(source) + self._complex_agents_from_op_complex(target)
